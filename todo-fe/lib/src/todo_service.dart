...
class TodoService {
  ...
  Future<Response> getAll() async {
    return authenticated().then((headers) => _http.get(_todosApi(), headers: headers));
  }

  Future<Response> create(String text) async {
    return authenticated().then((headers) => _http.post(_todosApi(), headers: headers, body: json.encode({'text': text})));
  }

  Future<Response> update(Todo todo) async {
    final url = '${_todosApi()}/${todo.id}';
    return authenticated().then((headers) => _http.put(url, headers: headers, body: json.encode(todo)));
  }

  Future<Response> delete(int id) async {
    final url = '${_todosApi()}/$id';
    return authenticated().then((headers) => _http.delete(url, headers: headers));
  }
